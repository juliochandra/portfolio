...
- **Aturan dependensi:** presentation → application → domain; infrastructure
  ...
  middleware jadi lebih sederhana (tanpa pengecualian manual untuk rute
  login seperti lazimnya dibutuhkan bila login ikut di bawah prefix
  terlindung).
...
│   ├── skills/                   ← F-01.2, F-06.4 (ENT-04, EP-11, EP-17, SA-07..09)
│   ├── tags/                     ← F-06.8 (ENT-03, EP-14, SA-16..18) — folder baru v2.2
...
|--------|-------|----------|
| `features/projects/` | domain · application (+ Server Action) · infrastructure · presentation | F-01.2, F-03, F-06.2 |
| `features/posts/` | domain · application (+ Server Action) · infrastructure · presentation | F-01.2, F-04, F-06.3 |
| `features/skills/` | domain · application (+ Server Action) · infrastructure · presentation | F-01.2, F-06.4 (+ EP-17 baca publik, v2.3) |
| `features/tags/` | domain · application (+ Server Action) · infrastructure · presentation | F-06.8 |
| `features/media/` | domain · application (+ Server Action) · infrastructure · presentation | F-06.9 |
| `features/contact/` | domain · application (+ Server Action) · infrastructure · presentation | F-05.1, F-06.5 |
| `features/messages/` | domain · application (+ Server Action) · infrastructure · presentation | F-05.2, F-06.7 |
| `features/auth/` | domain · application (+ Server Action) · infrastructure · presentation | F-06.1, F-06.6, F-06.10 |
| `features/dashboard/` | application (query agregat saja, tanpa domain/entitas sendiri) · presentation | SCR-09 (pemanis, non-blocking) |
| `app/` | presentation (route & Route Handlers) | SCR-01..SCR-19, EP-01..EP-17 |
...
- **Route (`app/`) tipis** — halaman publik/admin memanggil use case
  (`application/`) fitur terkait untuk baca data; form admin memanggil
  **Server Action** (`application/actions.ts`, ditandai `"use server"`)
  langsung dari `action` form, tanpa lewat Route Handler (D-012). Logika
  (aturan wajib/opsional, penghapusan permanen, transisi status) tinggal di
  `domain`/`application` fiturnya, bukan di route atau di action itu sendiri
  — action hanya memvalidasi input (Zod) & memanggil use case.
...
- **Akses berkas** (baca/tulis gambar unggahan) hanya dari `infrastructure/`
  ...
  ditautkan langsung dari komponen Hero (pm_01 D007).
- **Dashboard** (`features/dashboard/`) hanya lapisan `application` (query
