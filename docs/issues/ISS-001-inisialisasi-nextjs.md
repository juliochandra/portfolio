...
| **Ukuran** | M |
| **Blocked by** | — |
| **Serves** | Framework, UI Library, Bahasa, Styling, Icon, Runtime, API Layer, Validasi, Form, Test, Linter/Formatter |
| **Covers** | — |
...
Form, test Vitest + Playwright, linter Biome) sebelum baris kode fitur
pertama ditulis. Seluruh issue backend & frontend proyek ini menunggu
fondasi ini selesai — tanpa kerangka aplikasi, use case dan Server Action
tidak punya tempat berjalan. **Workflow GitHub Actions (CI) sengaja
ditunda** — proyek masih dikembangkan lokal, bukan bagian issue ini
(Titipan backlog).
...
- [ ] Akun & akses GitHub tersedia (untuk repositori).
...
- Workflow GitHub Actions (CI, atau Docker/Caddy/Cloudflare/deploy) —
  ditunda ke akhir backlog, Titipan (belum ada nomor issue).

## Konfigurasi

Tidak ada variabel env/kredensial di issue ini — `DATABASE_URL` disiapkan
di ISS-002 (koneksi database), `R2_*` (penyimpanan berkas) di ISS-003
(bucket R2), bukan bagian inisialisasi proyek ini.
...
4. `npm run test`
5. `npm run build`
6. Review struktur folder (`src/app`, `src/features`, `src/shared`).
...
- [ ] Struktur folder dasar kosong (`src/app/`, `src/features/`, `src/shared/`).
...
- Workflow GitHub Actions (CI), Docker, Caddy, Cloudflare CDN, deploy —
  ditunda ke akhir backlog (Titipan, belum ada nomor issue; proyek masih
  dikembangkan lokal).
...
- [ ] `npm run build` sukses tanpa error.
- [ ] `npm run lint` (Biome) sukses tanpa error.
- [ ] `npm run test` (Vitest + Playwright) sukses tanpa error.
- [ ] Struktur folder dasar (`src/app`, `src/features`, `src/shared`) sudah ada.
