...
import type { DashboardSummary } from "@/features/dashboard/dashboard.type";
		...
		<div className="space-y-8">
			<section aria-label="Ringkasan Statistik" className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4">
				<StatCard
					...
					description={`Published: ${summary.publishedPosts}`}
				/>
				<StatCard
					icon={FaBriefcase}
					label="Total Projects"
					value={String(summary.totalProjects)}
					description={`Published: ${summary.publishedProjects}`}
				/>
				<StatCard icon={FaTag} label="Total Tags" value={String(summary.totalTags)} description="Available tags" />
				<StatCard icon={FaCode} label="Total Skills" value={String(summary.totalSkills)} description="Available skills" />
			</section>
